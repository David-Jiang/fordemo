  ...
  portfolio.ajax = function () {
        ...
        if (newHash.substr(0, rootLength) == root) {
          ...
          $mainContent.load(
            ...
            function (xhr, statusText, request) {
              ...
              if (statusText == "error") {
                $mainContent.html(
                  '<div class="row pad-top pad-bottom"><div class="col-md-12 pad-top pad-bottom"><div class="alert-message error"><p>The Content cannot be loaded.</p></div></div></div>'
                );
                $pageWrap.find("#preloader").remove();
              }
              closeProject();
          ...
          $("#portfolio-items a[href='" + newHash + "']")
            .parent()
            .addClass("current");
          var projectIndex = $("#portfolio-items")
            .find("article.current")
            .index();
    ...
    function nextProject() {
      $("#next-project").on("click", function () {
        $(".single-portfolio").remove();
        window.location.hash = $("#portfolio-items .current")
          .next()
          .find("a")
          .attr("href");
        return false;
